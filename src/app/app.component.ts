...
export class AppComponent implements OnInit {
    ...
    for (let key in data) {
      ...
      if (key == "avgTeamWork") {
        for (let i = 0; i < data["lstContractProcessed"].length; i++) {
          series.push({ name: data["lstContractProcessed"][i].timeScale, value: data["avgTeamWork"] });
        }
        temp.push({ "name": key, "series": series });
      } else {
        for (let i = 0; i < data[key].length; i++) {
          series.push({ name: data[key][i].timeScale, value: data[key][i].contractProcessed })
        }
        temp.push({ "name": key, "series": series });
      }
    }
  ...
  formatCompletionStatusData(data) {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      for (let key in data[i]) {
        if (key != "timeScale") {
          series.push({ "name": key, "value": data[i][key] });
        }
      }
      temp.push({ name: data[i].timeScale, series: series });
    }
    return temp;
  }
  ...
  formatTurnaroundTimeData(data) {
    ...
    for (let i = 0; i < data.lstTurnaroundTime.length; i++) {
      temp.push({ "name": data.lstTurnaroundTime[i].userInfo, "value": data.lstTurnaroundTime[i].avgTime });
    }
    return temp;
  }
    ...
    for (let key in data) {
      ...
      if (key == "avgTeamError") {
        for (let i = 0; i < data["lstAnalystErrors"].length; i++) {
          series.push({ name: data["lstAnalystErrors"][i].userInfo, value: data["avgTeamError"] });
        }
        temp.push({ "name": key, "series": series });
      } else {
        if (key != "timeScale") {
          for (let i = 0; i < data[key].length; i++) {
            series.push({ name: data[key][i].userInfo, value: data[key][i].empError })
          }
          temp.push({ "name": key, "series": series });
        }
      }
    ...
    for (let key in data) {
      ...
      if (key == "avgTeamSLA") {
        for (let i = 0; i < data["lstSLAPerformance"].length; i++) {
          series.push({ name: data["lstSLAPerformance"][i].userInfo, value: data["avgTeamSLA"] });
        }
        temp.push({ "name": key, "series": series });
      } else {
        if (key != "timeScale") {
          for (let i = 0; i < data[key].length; i++) {
            series.push({ name: data[key][i].userInfo, value: data[key][i].empSLA })
          }
          temp.push({ "name": key, "series": series });
        }
      }
  ...
  formatBookOfWorkData(data) {
    let temp = [];
    for (let key in data) {
      temp.push({ "name": key, "value": data[key] });
    }
    return temp;
  }
}
